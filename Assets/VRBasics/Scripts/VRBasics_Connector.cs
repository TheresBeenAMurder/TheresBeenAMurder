...
public class VRBasics_Connector : MonoBehaviour {
	...
	//does the object's rigid body become kinematic when connected
	public bool kinematicWhenConnected = false;
	...
	void AttachMaleToFemale_whileMaleGrabbed(){

		GameObject grabParent;
		grabParent = transform.parent.transform.parent.gameObject;

		if (transform.parent.GetComponent<VRBasics_Grabbable> ().GetOriginalParent () == null) {
			transform.parent.transform.parent = null;
		} else {
			transform.parent.transform.parent = transform.parent.GetComponent<VRBasics_Grabbable> ().GetOriginalParent ().transform;
		}

		AttachMaleToFemale ();

		transform.parent.transform.parent = grabParent.transform;
	}
	...
	public void ConnectTo() {
		...
		if (type == connectorType.male && touching != null && !inTimeOut) {
			...
			if (transform.parent.GetComponent<VRBasics_Grabbable> ()) {
				...
				//the male is currently being grabbed
				} else {

					//is the female touching the male grabbable
					if (touching.transform.parent.GetComponent<VRBasics_Grabbable> ()) {

						//the female touching the male in not currently being grabbed
						if (!touching.transform.parent.GetComponent<VRBasics_Grabbable> ().GetIsGrabbed ()) {
							AttachMaleToFemale_whileMaleGrabbed ();
						}
					} 
				}
	...
	public void DetachFrom() {
		...
		if (type == connectorType.male && attachedTo != null) {
			...
			//return the male object to it's orginal parent
			if (transform.parent.GetComponent<VRBasics_Grabbable> ()) {

				if (transform.parent.GetComponent<VRBasics_Grabbable> ().GetIsGrabbed () == false) {
		...
		//female
			...
			if (attachedTo.transform.parent.GetComponent<VRBasics_Grabbable> ()) {
				...
				if (attachedTo.transform.parent.GetComponent<VRBasics_Grabbable> ().GetIsGrabbed () == false) {

					if (attachedTo.transform.parent.GetComponent<VRBasics_Grabbable> ().GetOriginalParent () == null) {
						attachedTo.transform.parent.transform.parent = null;
					} else {
						attachedTo.transform.parent.transform.parent = attachedTo.transform.parent.GetComponent<VRBasics_Grabbable> ().GetOriginalParent ().transform;
					}
				}
			}
		}
	...
	void KinematicChange(){
		if (type == connectorType.male) {
			if (isConnected && kinematicWhenConnected) {
				if (transform.parent.GetComponent<Rigidbody> ().isKinematic != true) {
					transform.parent.GetComponent<Rigidbody> ().isKinematic = true;
				}
			} else {
				if (transform.parent.GetComponent<Rigidbody> ().isKinematic != false) {
	...
	*/
	...
	IEnumerator ConnectionTimeOut(){
		...
		if (type == connectorType.female) {
			...
			//play the detach audio
			if (prevAttachedTo.GetComponent<VRBasics_Connector> ().detachAudio) {
				prevAttachedTo.GetComponent<VRBasics_Connector> ().detachAudio.Play ();
			}
